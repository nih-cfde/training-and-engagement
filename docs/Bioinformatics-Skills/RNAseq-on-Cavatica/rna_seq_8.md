...
* In the popup box, select <span class="highlight_txt">RStudio</span> for Environment. Provide an analysis name in the box. We used Cancer_DGE in this lesson. Click <span class="highlight_txt">Next</span> when done.
* In the `Compute requirements` tab, we will use the default instance type (c5.2xlarge, $0.48/hr). We increase the `Suspend time`, which is the period of inactivity after which the instance is stopped automatically and the analysis is saved, from 30 to 60 minutes. Click <span class="highlight_txt">Start the analysis</span>. This prompts the initializing of the analysis which involves set up of the instance and  preparing analysis environment.
...
!!! info "RStudio IDE"
    ...
    [Read more about the different panes and options of the RStudio interface](https://georgejmount.com/tourofrstudio/), the integrated development environment for the R programing language
...
To upload the script file, click the `Upload` option in the Files/Packages/Plots/Help/Viewer pane and select `Choose File` to select the file from your local computer. Once uploaded, click on the script file name to open it in the script editor pane (top left hand corner).
...
Once completed, you will get a popup window asking to try to open the html report. Click </br>`Try Again` to open a new tab for the report.

![Popup window](../rna-seq-images/rna-seq-8-2.png "Popup window")

Alternatively, you can click `Cancel` in the popup window and subsequently click <span class="highlight_txt">Stop</span> to view the files in your project folder.
...
You can also execute the code by selecting chunks or line by line and clicking the `Run` option or using ++ctrl+enter++ keys. This allows you greater flexibility to explore and understand the data.

![Use Run](../rna-seq-images/rna-seq-8-4.png "Use Run")

This version include the Bioconductor package, [`pcaExplorer`](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-019-2879-1) which provides interactive visualization of RNA-Seq datasets based on Principal Components Analysis.
...
All the files are clickable for preview on Cavatica. You can either download individual files by clicking on the file name or follow the steps to tag and download the files listed in the [Analysis with DESeq2 Public App](./rna_seq_7.md) lesson.
